...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = checkRateLimit(`mentor:${clientId}`, { windowMs: 60000, maxRequests: 5 })
  ...
  try {
    ...
    const result = await db.insert(applications).values({
      ...
      message: data.whyMentor || `Expertise: ${data.expertiseAreas}, Availability: ${data.availability}`,
    ...
    trackConversion({
      ...
      conversionName: 'Mentor Application',
    ...
    upsertCrmContact({ fullName: data.fullName, email: data.email, formType: 'mentor', applicationId: result[0].id })
