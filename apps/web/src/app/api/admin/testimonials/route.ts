...
export async function GET() {
  ...
  try {
    const data = await db.select().from(testimonials).orderBy(asc(testimonials.displayOrder))
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await auditLog({
      ...
      entityId: newTestimonial[0].id,
      action: "create",
      details: `Created testimonial from: ${data.name}`,
